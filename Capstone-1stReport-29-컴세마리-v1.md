...
| **(4) 주요 기능의 구현** | **1.추천 시스템** <br><br> 1-1.사용 라이브러리 <br> - scikit-learn : cosine_similarity 메서드를 활용하여 키워드 형식의 사용자 취향 데이터와 아이템(여가) 간의 유사도를 계산합니다. <br> - 서프라이즈 (Surprise) : 사용자-아이템 평점 데이터를 분석한 뒤, SVD(행렬 분해) 기법을 통해 각 사용자와 아이템의 잠재적 요인을 추출합니다. 이를 기반으로 사용자가 평가하지 않은 아이템의 평점을 예측합니다. <br><br> 1-2.구현 과정 <br>    a. 초기 단계 - 데이터가 부족해 협력 필터링 추천이 어려운 경우 <br>     1.사용자 취향 데이터 수집- 챗봇과 사용자의 대화를 통해 사용자의 취향 데이터(키워드)를 수집해 사용자 취향 프로필을 작성합니다. <br>     2.유사도 계산- 각 아이템(영화/공연/전시)들과 사용자 취향 데이터 간의 유사도를 계산합니다. <br>     3.추천 리스트 생성- 높은 유사도를 보이는 아이템들로 추천 리스트를 만들어 출력합니다. 유사도 계산 결과가 대체로 낮게 나와 양적으로 충분한 추천 리스트를 생성하지 못하면 일반적으로 높은 선호도를 보이는(인기 있는) 여가를 함께 추천합니다. <br><br>    b. 평점 데이터가 충분히 모여 협력 필터링 추천이 가능한 경우 <br>     1.SVD 모델 학습 및 예측- 평점 데이터를 기반으로 서프라이즈 패키지 내의 SVD 모델을 학습합니다. 학습된 모델을 이용해 각 아이템 별 사용자의 예측 평점을 계산합니다. <br>     2.추천 후보군 추출- 예측 평점은 사용자가 관심을 가질 가능성이 높은 아이템을 찾는 데 사용되며, 이를 바탕으로 상위 아이템을 추천 후보군으로 추출합니다. <br>     3.유사도 기반 최종 추천- 이전 단계(a-2)에서 계산된 코사인 유사도를 활용해 추출된 추천 후보군 아이템 중 높은 유사도를 가지는 아이템을 추천 리스트에 넣어 출력합니다. <br><br><br> **2.사용자 취향 데이터 수집용 챗봇** <br><br> 1-1.사용 API : Open AI API (GPT 3.5-turbo) <br><br> 1-2.대화 흐름 설계 : 사용자의 취향과 선호 정보를 수집할 수 있도록 적절한 질문을 제시합니다. 초기 단계에서는 일반적이고 포괄적인 질문을 통해 사용자의 관심사를 탐색하며, 이후 사용자의 응답 내용을 기반으로 구체화된 추가 질문을 생성합니다. <br><br> 1-3.자연어 처리를 활용한 감성 분석 및 긍정 키워드 추출 : 감성 분석 기술을 활용하여 사용자의 응답에서 긍정적인 반응을 나타내는 키워드를 추출하고, 사용자 취향 프로필에 저장합니다. <br>|
| **(5) 기타** | *기타 사항을 기술* |
